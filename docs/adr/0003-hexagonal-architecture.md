...
- **Ports** — abstractions for inbound email, PMS access, and persistence. The current set evolves with the implementation; see `services/email-processor/domain/ports.py` for the live list.
- **Adapters** — concrete implementations of each port, located under `services/email-processor/adapters/`. Each adapter encapsulates one external provider (Microsoft Graph, Mews, Postgres, Anthropic, etc.).
